...
void key_scheduling_IMP(uint8_t *MK, uint8_t *RK) __attribute__((optimize("-O3")));
void block_encryption_IMP(uint8_t *PT, uint8_t *RK, uint8_t *CT) __attribute__((optimize("-O3")));
void CTR_mode_IMP(uint8_t *PT, uint8_t *MK, uint8_t *CT, uint8_t num_enc) __attribute__((optimize("-O3")));
void POLY_MUL_RED_IMP(uint8_t *IN1, uint8_t *IN2, uint8_t *OUT) __attribute__((optimize("-O3")));
void AUTH_mode_IMP(uint8_t *CT, uint8_t *AUTH, uint8_t num_auth) __attribute__((optimize("-O3")));
void ENC_AUTH_IMP(uint8_t *PT, uint8_t *MK, uint8_t *CT, uint8_t *AUTH, uint8_t length_in_byte) __attribute__((optimize("-O3")));
...
void key_scheduling_IMP(uint8_t *MK, uint8_t *RK)
{
    ...
    *(uint64_t *)RK = *(uint64_t *)MK;
    ...
    for (i = 1; i < NUM_ROUND; i++)
    {
        RK[i * 8 + 0] = ROL(RK[(i - 1) * 8 + 0], (i + OFFSET1) % 8) + cache[(i * 8 + 0) % 64];
        RK[i * 8 + 1] = ROL(RK[(i - 1) * 8 + 1], (i + OFFSET5) % 8) + cache[(i * 8 + 1) % 64];
        RK[i * 8 + 2] = ROL(RK[(i - 1) * 8 + 2], (i + OFFSET1) % 8) + cache[(i * 8 + 2) % 64];
        RK[i * 8 + 3] = ROL(RK[(i - 1) * 8 + 3], (i + OFFSET5) % 8) + cache[(i * 8 + 3) % 64];
        RK[i * 8 + 4] = ROL(RK[(i - 1) * 8 + 4], (i + OFFSET1) % 8) + cache[(i * 8 + 4) % 64];
        RK[i * 8 + 5] = ROL(RK[(i - 1) * 8 + 5], (i + OFFSET5) % 8) + cache[(i * 8 + 5) % 64];
        RK[i * 8 + 6] = ROL(RK[(i - 1) * 8 + 6], (i + OFFSET1) % 8) + cache[(i * 8 + 6) % 64];
        RK[i * 8 + 7] = ROL(RK[(i - 1) * 8 + 7], (i + OFFSET5) % 8) + cache[(i * 8 + 7) % 64];
    }
}
...
void block_encryption_IMP(uint8_t *PT, uint8_t *RK, uint8_t *CT)
{
    ...
    *(uint64_t *)intermediate = *(uint64_t *)PT;

    for (i = 0; i < NUM_ROUND; i++)
    {
        intermediate[7] = ROL((uint8_t)(RK[i * 8 + 7] ^ (intermediate[6] + (RK[i * 8 + 7] ^ intermediate[7]))), 7);
        intermediate[6] = ROL((uint8_t)(RK[i * 8 + 6] ^ (intermediate[5] + (RK[i * 8 + 6] ^ intermediate[6]))), 6);
        intermediate[5] = ROL((uint8_t)(RK[i * 8 + 5] ^ (intermediate[4] + (RK[i * 8 + 5] ^ intermediate[5]))), 5);
        intermediate[4] = ROL((uint8_t)(RK[i * 8 + 4] ^ (intermediate[3] + (RK[i * 8 + 4] ^ intermediate[4]))), 4);
        intermediate[3] = ROL((uint8_t)(RK[i * 8 + 3] ^ (intermediate[2] + (RK[i * 8 + 3] ^ intermediate[3]))), 3);
        intermediate[2] = ROL((uint8_t)(RK[i * 8 + 2] ^ (intermediate[1] + (RK[i * 8 + 2] ^ intermediate[2]))), 2);
        intermediate[1] = ROL((uint8_t)(RK[i * 8 + 1] ^ (intermediate[0] + (RK[i * 8 + 1] ^ intermediate[1]))), 1);
    ...
    *(uint64_t *)CT = *(uint64_t *)intermediate;
}
...
void CTR_mode_IMP(uint8_t *PT, uint8_t *MK, uint8_t *CT, uint8_t num_enc)
{
    ...
    *(uint64_t *)intermediate = 0xdebc9a7856341200ull;

    for (i = 0; i < num_enc; i++)
    {
        intermediate[0] = ctr++;
        block_encryption_IMP(intermediate, RK, intermediate2);
        ((uint64_t *)CT)[i] = ((uint64_t *)PT)[i] ^ *(uint64_t *)intermediate2;
    }
}
...
void AUTH_mode_IMP(uint8_t *CT, uint8_t *AUTH, uint8_t num_auth)
{
    ...
    *(uint64_t *)AUTH_nonce = 
        ((uint64_t)num_auth << 56) | ((uint64_t)num_auth << 48) | ((uint64_t)num_auth << 40) | ((uint64_t)num_auth << 32) | 
        ((uint64_t)num_auth << 24) | ((uint64_t)num_auth << 16) | ((uint64_t)num_auth <<  8) |  (uint64_t)num_auth;
    *(uint64_t *)AUTH_nonce ^= 0xde00007800001200ull;
    *(uint64_t *)AUTH_nonce |= 0x00bc000056000000ull;
    *(uint64_t *)AUTH_nonce &= 0xffff9affff34ffffull;

    POLY_MUL_RED_IMP(AUTH_nonce, AUTH_nonce, AUTH_inter);

    for (i = 0; i < num_auth; i++)
    {
        *(uint64_t *)AUTH_inter ^= ((uint64_t *)CT)[i];
        POLY_MUL_RED_IMP(AUTH_nonce, AUTH_inter, AUTH_inter);
        POLY_MUL_RED_IMP(AUTH_inter, AUTH_inter, AUTH_inter);
    }

    *(uint64_t *)AUTH = *(uint64_t *)AUTH_inter;
}
