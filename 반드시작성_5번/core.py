...
def encode_block(block, derived_keys, encryption, enable_permute = True, round_num = 0):
    if enable_permute: block = permute(block, 64, INITIAL_PERMUTATION)

    block = block >> 32, block & 0xffffffff

    if not encryption: derived_keys = reversed(derived_keys)

    if round_num == 0:
        round_num = len(derived_keys)

    for key in derived_keys[:round_num]: 
        block = block[1], block[0] ^ f(block[1], key)

    block = block[1] << 32 | block[0]

    if enable_permute: block = permute(block, 64, INVERSE_PERMUTATION)
